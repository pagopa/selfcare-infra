<policies>
    <inbound>
      ...
      return $"Bearer {jwtHeaderBase64UrlEncoded}.{jwtPayloadBase64UrlEncoded}.{jwtSignatureBase64UrlEncoded}";
        ...
        <set-variable name="taxId" value="@{
          return ((JObject)context.Variables["requestBody"])["taxId"].ToString();
        }" />
        <set-variable name="vatNumber" value="@{
          return ((JObject)context.Variables["requestBody"])["vatNumber"].ToString();
        }" />
       <set-variable name="queryParams" value="@{
          var taxId = context.Variables.GetValueOrDefault<string>("taxId", null);
          var vatNumber = context.Variables.GetValueOrDefault<string>("vatNumber", null);
          return $"taxId={taxId}&vatNumber={vatNumber}";
        }" />
        <set-backend-service base-url="${BACKEND_BASE_URL}" />
        <rewrite-uri template="/national-registries/verify-legal?@(context.Variables["queryParams"])" />
    </inbound>
    <backend>
