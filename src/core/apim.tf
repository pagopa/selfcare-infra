...
module "apim_uservice_party_process_v1" {
  ...
  service_url = (var.env == "dev" || var.env == "uat") ? format("http://%s/ms-core/v1", var.reverse_proxy_ip) : format("http://%s/party-process/v1", var.reverse_proxy_ip)

  content_format = "openapi"
  content_value = templatefile("./api/party_process/v1/open-api.yml.tpl", {
    host     = azurerm_api_management_custom_domain.api_custom_domain.proxy[0].host_name
    basePath = (var.env == "dev" || var.env == "uat") ? "ms-core/v1" : "party-process/v1"
  })
...
module "apim_uservice_party_management_v1" {
  ...
  service_url = (var.env == "dev" || var.env == "uat") ? format("http://%s/ms-core/v1", var.reverse_proxy_ip) : format("http://%s/party-management/v1", var.reverse_proxy_ip)

  content_format = "openapi"
  content_value = templatefile("./api/party_management/v1/open-api.yml.tpl", {
    host     = azurerm_api_management_custom_domain.api_custom_domain.proxy[0].host_name
    basePath = (var.env == "dev" || var.env == "uat") ? "ms-core/v1" : "party-management/v1"
  })
