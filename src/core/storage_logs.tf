...
resource "azurerm_key_vault_secret" "selc_logs_storage_access_key" {
  ...
  value        = module.selc-logs-storage.primary_access_key
...
resource "azurerm_key_vault_secret" "selc_logs_storage_connection_string" {
  ...
  value        = module.selc-logs-storage.primary_connection_string
  content_type = "text/plain"
...
resource "azurerm_key_vault_secret" "selc_logs_storage_blob_connection_string" {
  ...
  value        = module.selc-logs-storage.primary_blob_connection_string
  content_type = "text/plain"

  key_vault_id = module.key_vault.id
}

resource "azurerm_storage_container" "selc-logs-container" {
  name                  = "${local.project}-logs-blob"
  storage_account_name  = module.selc-logs-storage.name
  container_access_type = "private"
}

module "logs_storage_snet" {
  source                                         = "git::https://github.com/pagopa/azurerm.git//subnet?ref=v2.12.1"
...
resource "azurerm_private_endpoint" "logs_storage" {
  ...
  private_service_connection {
    ...
    private_connection_resource_id = module.selc-logs-storage.id
