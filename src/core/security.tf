                ...
                "keys": [
                  {
                    ...
                    "use": "sig",
                    "x5c": [
                      "${azurerm_api_management_certificate.jwt_certificate.data}"
                    ],
                    "kid": "selfcare",
                    "x5t": "${azurerm_api_management_certificate.jwt_certificate.thumbprint}"
                  }
                ]}' > ${path.module}/.terraform/tmp/jwks.json
              az storage blob upload \
                --container '$web' \
                --account-name ${replace(replace(module.checkout_cdn.name, "-cdn-endpoint", "-sa"), "-", "")} \
                --account-key ${module.checkout_cdn.storage_primary_access_key} -s "${path.module}/.terraform/tmp/jwks.json" \
